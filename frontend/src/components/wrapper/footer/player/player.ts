...
import { AbstractSmart } from "@components/abstractSmart";
...
(window as any).currentTrackGlobal = null;

export const URL = "http://localhost:3000";

export class Player extends AbstractSmart {
	...
	constructor(private readonly tracks: ITrack[]) {
		super();
		this.data = this.updateData(this.tracks, null);
		this.eventEmitter = new Events();
		this.audioContext = null;
		this.audioSource = null;
		this.playerModel = new PlayerModel({
			currentTrack: this.data[0],
      ...
      <div class="player flex">
        <div class="player__track-name flex">
          <img
            ...
            alt="${state.currentTrack.artist.name}"
          />
          <div class="player__track-name__content">
            <div class="flex player__name__header">
              <h3 class="player__track__h3">${state.currentTrack.name}</h3>
              <button class="player__track__like" data-track-id="${
        ...
        <div class="player__controls" id="playerControls">
            ...
            <button class="player__play-btn">
              ...
              >
                <path
                  ...
                  fill="#AAAAAA"
                />
                <path class="player__play-btn--path"
          ...
          <div class="player__controls__footer" id="elapsedTime">
            <span class="player__time-start">${formatDuration(
							state.currentTime / 1000,
						)}</span>
            <div class="player__range-play" id="range-play">
              <div class="player__progress" id="progress"></div>
            </div>
            <span class="player__time-end">${formatDuration(
        ...
        <div class="player__value">
          ...
          <div class="player__value-range" id="range-value">
            <input type="range" class="player__volume-range" id="volume-range" min="0" max="100" step="5">
          </div>
        </div>
      </div>
    `;
	...
	removeElement() {
		this.element = document.querySelector(".player") ?? null;
	...
	repairHandlers() {
		this.setupEventLike();
		this.setupEventShuffle();
		this.setupEventPrev();
		this.setupEventPlay();
		this.setupEventNext();
		this.setupEventRepeat();
		this.renderProgress();
	}

	private renderPlayer() {
		const image = document.querySelector(".player__track__img");
		if (image) {
			image.setAttribute("src", this.playerModel.getState().currentTrack.image);
			image.setAttribute(
				"alt",
				this.playerModel.getState().currentTrack.artist.name,
			);
		}

		const trackName = document.querySelector(".player__track__h3");
		if (trackName) {
			trackName.innerHTML = this.playerModel.getState().currentTrack.name;
		}

		const artist = document.querySelector(".player__track__author");
		if (artist) {
			artist.innerHTML = this.playerModel.getState().currentTrack.artist.name;
		}

		const btnLike = document.querySelector(".player__track__like");
		if (btnLike) {
			btnLike.setAttribute(
				"data-track-id",
				this.playerModel.getState().currentTrack.id.toString(),
			);

			if (this.playerModel.getState().currentTrack.likes.length > 0) {
		...
		const endTime = document.querySelector(".player__time-end");
		if (endTime) {
			endTime.innerHTML = formatDuration(
				this.playerModel.getState().currentTrack.duration / 1000,
			);
		}
	}
	...
	private loadTrack(track: ITrack) {
		if (this.audioContext) {
			fetch(`${URL}` + track.path)
				.then((response) => response.arrayBuffer())
				.then((arrayBuffer) => this.audioContext.decodeAudioData(arrayBuffer))
				.then((audioBuffer) => this.playData(audioBuffer))
				.catch((error) => {
	...
	private playData(audioBuffer: AudioBuffer) {
		if (this.audioContext) {
			...
			this.updateVolume(this.playerModel.getState().volume);
			this.audioSource.start();
		}
	}

	private setupEventPlay() {
		const btnPlay =
			document.querySelector<HTMLButtonElement>(".player__play-btn");
		if (btnPlay) {
			btnPlay.onclick = () => {
				if (this.playerModel.getState().isPlaying) {
					this.stop();
				} else {
					this.play();
				}
	...
	private play() {
		const currentTrack =
			(window as any).currentTrackGlobal != null
				? (window as any).currentTrackGlobal
				: this.playerModel.getState().currentTrack;
		(window as any).currentTrackGlobal = null;

		const index = this.data.indexOf(currentTrack);
		const volumeRange = document.getElementById("volume-range");

		const tracksEl = Array.from(
			document.querySelectorAll<HTMLAnchorElement>(".track__name__link"),
		);
		if (tracksEl) {
			tracksEl.forEach((el) => {
		...
		if (index >= 0 && index < this.data.length) {
			...
			this.playerModel.setIsPlaying(true);
			(window as any).isPlayingGlobal = true;

			this.interval = setInterval(() => {
				if (this.playerModel.getState().isPlaying) {
					this.playerModel.setCurrentTime(
						(this.playerModel.getState().currentTime += 1000),
					);
					this.renderElapsedTime();
					this.renderProgress();

					if (
						this.playerModel.getState().currentTrack.duration <
						this.playerModel.getState().currentTime + 1000
					) {
						this.next();
					}
			...
			volumeRange.addEventListener("input", (event) => {
				...
				this.updateVolume(parseInt(volume, 10));
			});

			const pathBtnPlay = document.querySelector(".player__play-btn--path");
			pathBtnPlay.classList.add("active");
		}
	}
	...
	private stop() {
		if (this.audioSource) {
			...
			this.playerModel.setIsPlaying(false);
			(window as any).isPlayingGlobal = false;
			this.playerModel.setCurrentTime(0);
			clearInterval(this.interval);

			const pathBtnPlay = document.querySelector(".player__play-btn--path");
			pathBtnPlay.classList.remove("active");
		}
	}

	private setupEventPrev() {
		const button = document.querySelector<HTMLButtonElement>(
			".player__skipback-btn",
		);
		if (button) {
	...
	private prev() {
		const index = this.data.indexOf(this.playerModel.getState().currentTrack);

		if (index > 0) {
			this.stop();
			this.playerModel.setCurrentTrack(this.data[index - 1]);
			this.renderPlayer();
			this.play();
		}
	}

	private setupEventNext() {
		const button = document.querySelector<HTMLButtonElement>(
			".player__skipnext-btn",
		);
		if (button) {
			button.addEventListener("click", (event: MouseEvent) => {
				event.preventDefault();
	...
	private shuffle() {
		...
		while (uniqueIndexes.length < this.data.length) {
			let randomIndex = Math.floor(Math.random() * this.data.length);

			while (uniqueIndexes.includes(randomIndex)) {
		...
		this.playerModel.setCurrentTrack(this.data[0]);
		this.renderPlayer();
		this.play();
	}

	private setupEventRepeat() {
		const button = document.querySelector<HTMLButtonElement>(
			".player__repeat-btn",
		);
		if (button) {
	...
	private handleEventLikeClick(track: ITrack) {
		return async (event: MouseEvent) => {
			event.preventDefault();
			const target = event.currentTarget as HTMLElement;

			if (target.closest(".player__track__like")) {
	...
	private setupEventLike() {
		const button = document.querySelector(".player__track__like") ?? null;
		const callback = this.handleEventLikeClick(this.data[0]);
	...
	private renderElapsedTime() {
		const time = document.querySelector(".player__time-start");
		if (time) {
			time.innerHTML = formatDuration(
				this.playerModel.getState().currentTime / 1000,
			);
		}
	}
	...
	private renderProgress() {
		...
		const currentTime = this.playerModel.getState().currentTime;
		const duration = this.playerModel.getState().currentTrack.duration;
	...
	override updateElement(
		...
		// const currentPlaylist = this.playerModel.getState().currentPlaylist;
		// const playlistChanged = playlist !== currentPlaylist;
		// if (playlist === null) {
		...
		playlist != null
			? presenterPlayer(this.updateData(tracks, playlist))
			: presenterPlayer(this.updateData(tracks, null));

		if (playlist !== null) {
			this.playerModel.setCurrentPlaylist(playlist);
		} else {
			this.playerModel.setCurrentPlaylist(null);
		}

		if (track != null) {
			this.playerModel.setCurrentTrack(track);
			(window as any).currentTrackGlobal = track;
		} else {
			(window as any).currentTrackGlobal = null;
		}
	...
	override updateData(tracks: ITrack[], playlist: IPlaylist) {
		return playlist !== null ? playlist.songs : tracks;
	}
}
